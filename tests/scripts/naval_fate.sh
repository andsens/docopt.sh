...
if $_ship == 'true' && $_new == 'true'; then
  printf "Your new ship '%s' has been created.\n" "$__name_"
fi

if $_ship == 'true' && $_move == 'true'; then
  printf "The %s is now moving to %d,%d at %d knots.\n" "$__name_" "$__x_" "$__y_" "$___speed"
fi

if $_ship == 'true' && $_shoot == 'true'; then
  printf "You shoot at %d,%d. It's a hit!\n" "$__x_" "$__y_"
fi

if $_mine == 'true' && $_set == 'true'; then
  printf "A mine has been laid at %d,%d" "$__x_" "$__y_"
	if $___drifting; then
...
if $_mine == 'true' && $_remove == 'true'; then
